...
def test_attack_simulator():
    """测试攻击模拟器"""
    simulator = AttackSimulator(prob=1.0)  # 确保总是应用攻击
    simulator.train()  # 训练模式
    ...
    # 测试高斯模糊
    simulator.eval()
    blurred = simulator.gaussian_blur(x)
    print(f"高斯模糊输出形状: {blurred.shape}")
    
    # 测试噪声
    noisy = simulator.gaussian_noise(x)
    print(f"噪声输出形状: {noisy.shape}")
    
    # 测试JPEG压缩
    jpeg = simulator.jpeg_compression(x)
    print(f"JPEG输出形状: {jpeg.shape}")
    
    # 测试随机裁剪
    cropped = simulator.random_crop_resize(x)
    print(f"裁剪输出形状: {cropped.shape}")
    
    # 测试旋转
    rotated = simulator.random_rotate(x)
    print(f"旋转输出形状: {rotated.shape}")
    
    # 测试颜色抖动
    color = simulator.color_jitter(x)
    print(f"颜色抖动输出形状: {color.shape}")
    
    # 测试组合攻击
    combined = simulator.combined_attack(x, num_attacks=3)
    print(f"组合攻击输出形状: {combined.shape}")
