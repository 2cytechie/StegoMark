...
def test_attack_simulator():
    ...
    # 加载图片
    img_path = config.test_target_img
    ...
    # 创建攻击模拟器
    simulator = AttackSimulator(prob=1.0)
    simulator.train()  # 确保处于训练模式
    ...
    # 保存原始图片
    original_img = tensor_to_pil(img_tensor.squeeze(0))
    original_img.save(os.path.join(output_dir, 'original.jpg'))
    print("保存原始图片: original.jpg")
    ...
    for attack_type in attack_types:
        ...
        # 应用特定攻击
        if attack_type == 'blur':
            attacked = simulator.gaussian_blur(img_tensor)
        elif attack_type == 'noise':
            attacked = simulator.gaussian_noise(img_tensor)
        elif attack_type == 'jpeg':
            attacked = simulator.jpeg_compression(img_tensor)
        elif attack_type == 'crop':
            attacked = simulator.random_crop_resize(img_tensor)
        elif attack_type == 'rotate':
            attacked = simulator.random_rotate(img_tensor)
        elif attack_type == 'color':
            attacked = simulator.color_jitter(img_tensor)
        elif attack_type == 'dropout':
            attacked = simulator.dropout(img_tensor)
    
    # 转换回PIL图像并保存
    attacked_img = tensor_to_pil(attacked.squeeze(0))
    save_path = os.path.join(output_dir, f'{attack_type}.jpg')
    attacked_img.save(save_path)
    print(f"保存攻击后的图片: {attack_type}.jpg")
